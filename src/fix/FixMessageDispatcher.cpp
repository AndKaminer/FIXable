...
void FixMessageDispatcher::registerHandler(
    const std::string& msgType, std::unique_ptr<FixMessageHandler> h) {
  handlers[msgType] = std::move(h);
}

void FixMessageDispatcher::dispatch(const FixMessage& msg,
                                    FixSessionManager* session) const {
